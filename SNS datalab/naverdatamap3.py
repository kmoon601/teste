...
def get_request_url(url): # 데이터 요청하여 가져오기 - 크럴러 작업
    ...
    try:
        ...
        print(datetime.datetime.now(),"오류",url)
...
def mapByfolium(coord):
    tip = 'Check'

    mapData = folium.Map(location=coord, zoom_start=15) # coordinates
    mapData = folium.Marker(coord, popup='Check', tooltip=tip).add_to(mapData)
    mapData.save(r"C:\lab\map00.html")

    
def Main():
    addrData = GetGeoLocationData('필동로1길 30')
    # 구글 지도              # staticmap 으로 해라 () https://api.ncloud-docs.com/docs/ai-naver-mapsstaticmap-raster)
   

    addrData = GetGeoLocationData('주소')
    coord = [addrData['addresses'][0]['y'], addrData['addresses'][0]['x']]
